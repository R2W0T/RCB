...
CameraDriverBinNode::CameraDriverBinNode() : Node("camera_driver_bin_node") {
    ...
    image_subscriber = this->create_subscription<sensor_msgs::msg::Image>("raw_image", 10, std::bind(&CameraDriverBinNode::process_image_callback, this, _1));

        
}


void CameraDriverBinNode::process_image_callback(const sensor_msgs::msg::Image::SharedPtr msg) const {
...
void CameraDriverBinNode::encode_img(const cv::Mat &binary_img, std::vector<uint8_t> &encoded_img) const {
...
void CameraDriverBinNode::publish(cv::Mat &img) const {
    
    cv::Mat gray_img, binary_img;

    cv::cvtColor(img, gray_img, cv::COLOR_BGR2GRAY);
    cv::threshold(gray_img, binary_img, 75, 1, cv::THRESH_BINARY);
